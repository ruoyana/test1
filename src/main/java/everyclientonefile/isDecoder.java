...
public class isDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext channelHandlerContext, ByteBuf byteBuf, List<Object> list) throws Exception {
           ...
           //三元表达式 在if里最终结果是看赋值的值来判断的 不是看条件自己判断 br.get(channelHandlerContext)第一次进来执行完后赋了个false 表示有文件进来了然后等这个文件接完就变为true之后继续接收
        if (br.get(channelHandlerContext) == null ? true : br.get(channelHandlerContext)) { //每个客户端的文件 第一次进来都是null 进来后都会赋一次值
            ...
            if (mark == 1) {
                ...
                User user = new User();
                user.setMark(1);
                user.setFilename(new String(bytes));
                list.add(user);
                ...
                contentSumLength.put(channelHandlerContext,filelength);
                br.put(channelHandlerContext,false);

                User user = new User();
                user.setMark(2);
                user.setFilename(new String(bytes));
                user.setFilecontentLength(filelength);
                list.add(user);
